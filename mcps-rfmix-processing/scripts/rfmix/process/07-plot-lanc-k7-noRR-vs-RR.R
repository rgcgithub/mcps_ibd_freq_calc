...
# fixed
lanc = lapply(vals_chr, function(x) {
  glue('output/rfmix/processed/panel-oxford_mais_k{K}_RR/param-rbase/lanc-vcf-raw/lanc.unrel.chr{x}.tsv.gz') %>%
    fread %>% mutate(chr = x)
}) %>% bind_rows
if(K == 7) { lanc = mutate(lanc, AMERICA = 1 - AFRICA - EUROPE) }
...
g = plot_grid(
  fun_plot(tab) + labs(title = 'noRR (default)') + theme(legend.position = 'none'),
  fun_plot(ftab) + labs(title = 'RR') + theme(legend.position = 'none'),
